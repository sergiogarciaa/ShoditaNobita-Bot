...
#EMAIL: SERGIOOGARCIABARRERA@GMAIL.COM     ##
#############################################
...
## Obtención de puertos a través de archivo
abrirArchivo = open("dic/ports.txt", "r")   #Insertamos ruta del archivo y la guarda
cargaArchivo = abrirArchivo.readlines()     # Lo lee (String)

# STRING TO INT, necesario para la lectura de .socket (L150)
archivoInt = [eval(i) for i in cargaArchivo]
print("Lista modificada: ", archivoInt)
...
def banner_grabbing(ip_address, port):
    global archivoInt
# .archivoInt es una lista de tripo String, .socket funciona con lista de INT.
    try:
        s=socket.socket()
        s.settimeout(5.0)
        s.connect((ip_address, port))
...
def porcentaje(portID):
    global archivoInt, nPuertos
    total = (portID + 1) * 100
    total = total / nPuertos
    return total
...
def main():
    global archivoInt, nPuertos, ip_address
    f = open("dic/victimitas.txt", "r")
    targets = f.readlines()
